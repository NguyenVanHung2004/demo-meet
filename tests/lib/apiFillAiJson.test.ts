...
describe("parseAiJson robustness (requestFillPlaceholders)", () => {
  ...
  it("parse JSON thuần (object)", async () => {
    ...
    const result = await api.requestFillPlaceholders(["TEN", "NGAY"]);
  ...
  it("bỏ extra text trước/sau JSON", async () => {
    ...
    const api = await importApi();
    const result = await api.requestFillPlaceholders(["TEN"]);
  ...
  it("AI trả mảng -> trả object rỗng (vì fill_placeholders cần object)", async () => {
    ...
    const api = await importApi();
    const result = await api.requestFillPlaceholders(["x"]);
  ...
  it("AI trả rỗng -> throw", async () => {
    mockFetch({ summary: "" });
    const api = await importApi();
    await expect(api.requestFillPlaceholders(["x"])).rejects.toThrow(/rỗng|không trả về/);
  });

  it("JSON hoàn toàn lỗi -> throw và log", async () => {
    mockFetch({ summary: "this is not json at all" });
    const api = await importApi();
    await expect(api.requestFillPlaceholders(["x"])).rejects.toThrow(/không hợp lệ/);
  });
});
...
describe("parseAiJson robustness (requestDetectFill)", () => {
  ...
  it("parse JSON thuần (array)", async () => {
    ...
    const api = await importApi();
    const result = await api.requestDetectFill("file text");
  ...
  it("bóc tách markdown code block", async () => {
    ...
    const api = await importApi();
    const result = await api.requestDetectFill("file");
  ...
  it("JSON lỗi hoàn toàn -> throw", async () => {
    ...
    const api = await importApi();
    await expect(api.requestDetectFill("file")).rejects.toThrow(/không hợp lệ/);
  });
});
