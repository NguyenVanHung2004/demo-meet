...
export default function EditorState({
  ...
  // [FIX] Khởi tạo duration từ data đã lưu (vì WebM Blob thường trả về Infinity)
  ...
  const formatTime = (time: number) => {
    // [FIX] Check finite để tránh Infinity:NaN
  ...
  const handleUpdateText = (id: string, newText: string) => {
    setSegments(prev => prev.map(s =>
      s.id === id ? { ...s, text: newText, words: [] } : s // words: [] để tắt karaoke câu này
    ));
  };
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      let finalAudioUrl = initialData.audioUrl;

      // [MỚI] Nếu là Draft -> Upload Audio lên Firebase trước
      if (initialData.status === 'draft' && audioSrc.startsWith('blob:')) {
        // 1. Lấy Blob từ URL
        const blob = await fetch(audioSrc).then(r => r.blob());
        const file = new File([blob], `${initialData.title || 'Draft'}.webm`, { type: 'audio/webm' });

        // 2. Upload
        toast.info("Đang tải audio lên Cloud...");
        finalAudioUrl = await import("../lib/api").then(m => m.uploadAudioToFirebase(file, initialData.userId));
      }
      ...
      const updatedMeeting = {
        ...
        speakers: speakers,
        title: title,
        audioUrl: finalAudioUrl, // Cập nhật URL thật
        status: initialData.status === 'draft' ? 'completed' : initialData.status // Đổi status nếu là draft
      };

      // 3. Lưu Firestore
      await saveMeeting(updatedMeeting as any);

      // 4. Nếu là Draft -> Xóa Local
      if (initialData.status === 'draft') {
        await import("../lib/indexedDB").then(m => m.deleteDraft(initialData.id));
      }
    ...
    <div className="flex flex-col h-screen bg-white relative font-sans text-slate-900">
      ...
      <div className="h-20 bg-white border-t px-4 md:px-8 flex items-center gap-4 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] z-20 shrink-0">
        ...
        <audio
          ...
          onLoadedMetadata={(e) => {
            // [FIX] Chỉ update nếu browser tính được duration chuẩn
