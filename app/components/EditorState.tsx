...
export default function EditorState({
  ...
  const templateStructRef = useRef<HTMLTextAreaElement>(null); // [MỚI] Ref cho textarea
  ...
  const handleCreateTemplate = async () => {
    ...
    try {
      ...
      setNewTemplateName(""); setNewTemplateDesc(""); setNewTemplateStructure("");
      loadTemplates();
    } catch (e) {
  ...
  const handleDeleteTemplate = async (tmplId: string) => {
    if (!user) return;
    if (await confirm({ title: "Xóa mẫu này?", message: "Không thể hoàn tác.", type: "danger" })) {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-4xl h-[85vh] rounded-2xl shadow-xl flex flex-col overflow-hidden">
            <div className="p-4 border-b flex justify-between items-center bg-slate-50">
              ...
              <button onClick={() => { setShowTemplateModal(false); setIsCreatingTemplate(false); }} className="p-2 hover:bg-slate-200 rounded-full"><X className="w-5 h-5 text-slate-400" /></button>
            </div>
                ...
                <div className="w-full md:w-1/3 border-r bg-slate-50 overflow-y-auto p-3 space-y-2">
                  ...
                  {templates.filter(t => !t.isCustom).map(t => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg border transition-all text-sm ${selectedTemplate.id === t.id ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-200' : 'bg-white border-slate-200 hover:border-indigo-300'}`}
                    >
                  ...
                  {templates.filter(t => t.isCustom).length === 0 && <p className="text-xs text-slate-400 px-2 italic">Chưa có mẫu nào.</p>}
                  {templates.filter(t => t.isCustom).map(t => (
                ...
                {isCreatingTemplate ? (
                  <div className="flex flex-col h-full space-y-4 animate-in slide-in-from-right duration-200">
                    ...
                    <div className="flex-1 flex flex-col">
                      <label className="block text-sm font-bold text-slate-700 mb-1">Cấu trúc Prompt (AI Instruction)</label>

                      {/* [MỚI] Toolbar hỗ trợ Markdown */}
                      <div className="flex flex-wrap gap-2 mb-2">
                        <button onClick={() => insertTemplateSnippet("## TÊN MỤC\n")} className="px-2 py-1 bg-slate-100 hover:bg-indigo-50 text-indigo-700 text-xs font-bold rounded border border-slate-200">+ Mục lớn</button>
                        <button onClick={() => insertTemplateSnippet("- Nội dung chi tiết...\n")} className="px-2 py-1 bg-slate-100 hover:bg-indigo-50 text-slate-700 text-xs font-medium rounded border border-slate-200">+ Ý chính</button>
                        <button onClick={() => insertTemplateSnippet("  - [ ] Action Item\n")} className="px-2 py-1 bg-slate-100 hover:bg-indigo-50 text-slate-700 text-xs font-medium rounded border border-slate-200">+ Việc cần làm</button>
                        <button onClick={() => insertTemplateSnippet("**In đậm** ")} className="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded border border-slate-200">In Đậm</button>
                      </div>

                      <textarea
                        ref={templateStructRef}
                        value={newTemplateStructure} onChange={e => setNewTemplateStructure(e.target.value)}
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                      <button onClick={() => setIsCreatingTemplate(false)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg">Hủy</button>
                      <button onClick={handleCreateTemplate} className="px-6 py-2 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 shadow-md">Lưu mẫu mới</button>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col h-full">
                    ...
                    <div className="mt-6 flex justify-end">
                      <button
                        ...
                        className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg flex items-center gap-2"
                      >
